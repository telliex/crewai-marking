...
def create_footer(
    ...
    db.add(f)
    db.commit()
    return RedirectResponse(f"/settings/footers/{f.id}/edit?msg=Footer created.", status_code=303)
...
def update_footer(
    ...
    db.commit()
    return RedirectResponse(f"/settings/footers/{f.id}/edit?msg=Footer saved.", status_code=303)
